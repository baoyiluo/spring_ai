package com.jialli.first_ai_project.rag.controller;
...
public class EmployeeHandbookController {
    ...
    private void calculateSimilarity(String message) {
        float[] embeddingMsg = embeddingModel.embed(message);
        log.info("Embedding for {} - {}", message, embeddingMsg);
        float[] embeddingForVocationPolicy = embeddingModel.embed("Vacation policy");
        log.info("Embedding for Vacation policy - {}", embeddingForVocationPolicy);
        double similarityResult = SimpleVectorStore.EmbeddingMath.cosineSimilarity(embeddingMsg, embeddingForVocationPolicy);
