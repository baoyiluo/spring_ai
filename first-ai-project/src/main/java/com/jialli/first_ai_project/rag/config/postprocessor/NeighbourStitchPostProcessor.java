package com.jialli.first_ai_project.rag.config.postprocessor;
...
public class NeighbourStitchPostProcessor implements DocumentPostProcessor {
    ...
    public List<Document> process(Query query, List<Document> documents) {
        ...
        List<Document> neighbors = getNeighborDocuments(targets);
        List<Document> merged = getMergedDocuments(documents, neighbors);
    ...
    private List<Document> getNeighborDocuments(Set<Target> targets) {
    ...
    private List<Document> getMergedDocuments(List<Document> documents, List<Document> neighbors) {
        List<Document> merged = new ArrayList<>(documents.size() + neighbors.size());
    ...
    private List<Document> dedupBySourceIndexPreserveOrder(List<Document> merged) {
        ...
        for (var doc: merged) {
            String source = String.valueOf(doc.getMetadata().get(RagConstants.SOURCE));
            Object indexObj = doc.getMetadata().get(RagConstants.CHUNK_INDEX);
