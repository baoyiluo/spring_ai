package com.jialli.first_ai_project.rag.config.preprocessor;
...
public class DomainSynonymTransformer implements QueryTransformer {
    ...
    public DomainSynonymTransformer(RagConfigData ragConfigData) {
        ...
        if (synonyms != null) {
            synonyms.forEach((k, v)->
                    this.replacements.put(Pattern.compile("\\b" + Pattern.quote(k) + "\\b", Pattern.CASE_INSENSITIVE), v));
        }
    }
    ...
    private String getCleanedText(String queryText) {
        String cleanedText = queryText.trim()
